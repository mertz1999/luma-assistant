...
class RunManager extends EventEmitter {
  ...
  private startClaudeExecution(runId: string, effectiveConfig: RunConfig, prompt: string, credentialEnv: Record<string, string> = {}): void {
    ...
    const knownSecretValues = Object.values(credentialEnv).filter((v): v is string => Boolean(v));
    // See runners/permission-mapping.ts:resolveClaudePermissionMode for
    // the rationale (sandbox-driven, not an unconditional bypass).
...
const agentScheduleManager = new AgentScheduleManager(runManager, (schedule, prompt, execution) => {
  ...
  try {
    const run = runManager.startRun({
      ...
      skills: schedule.runConfig.skills,
      agents: [],
      // Resolved against THIS schedule's own project (derived from
      // workspace, same as every other execution path) inside
      // runManager.startRun itself -- policy is re-evaluated at dispatch
      // time, never grandfathered from when the schedule was created.
      requestedCredentials: schedule.runConfig.requestedCredentials,
    });
