...
function ensureCodex() {
  ...
  if (name !== legacyDefaultName) {
    const legacy = run(codexPath, ["mcp", "get", legacyDefaultName]);
    if (legacy.status === 0) {
      const legacyUrl = currentUrl(`${legacy.stdout}\n${legacy.stderr}`);
      if (legacyUrl === url) {
        console.log(`[telegram-mcp] Removing legacy Codex MCP '${legacyDefaultName}' at ${url}`);
        const removed = run(codexPath, ["mcp", "remove", legacyDefaultName], { stdio: "inherit" });
        if (removed.status !== 0) process.exit(removed.status || 1);
      }
    }
  }
...
function ensureClaude() {
  ...
  if (help.status !== 0) {
    console.log("[telegram-mcp] Claude CLI not available; skipping Claude MCP registration.");
    return;
  }
  ...
  if (existing.status === 0) {
    console.log(`[telegram-mcp] Refreshing Claude MCP '${name}' to user scope at ${url}`);
    run(claudePath, ["mcp", "remove", name], { stdio: "inherit" });
  } else {
    console.log(`[telegram-mcp] Adding Claude MCP '${name}' at ${url} (user scope)`);
  }
